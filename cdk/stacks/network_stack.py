...
class NetworkStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, config: InfrastructureConfig, **kwargs) -> None:
        ...
        if config.vpc.automatic_subnet_creation:
            ...
            self.public_subnets = ec2.SubnetSelection(subnets=my_vpc.all_subnets.get('public'))
            self.rds_subnets = ec2.SubnetSelection(subnets=my_vpc.all_subnets.get('rds-aurora'))
            self.eks_private_nat_subnets = ec2.SubnetSelection(subnets=my_vpc.all_subnets.get('eks-private-nat'))
            self.rds_lambda_subnets = ec2.SubnetSelection(subnets=my_vpc.all_subnets.get('rds-lambda-secret'))
